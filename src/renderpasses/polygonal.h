 ...
 */
	...
	bool init(const Config& config) override {
		...
		// Compute and set lightIrradiance
		lightIrradiance = emitter.getPower() / emitter.area;

		// Loop over each vertex and store xyz components to emitterVertexData. 
	...
	void addShadows() {
		 ...
		 */
		v3f camAt = camera.camera_look_at;
		v3f camUp = camera.camera_up;
		v3f camEye = camera.camera_position;
		glm::mat4 inverseView = glm::inverse(glm::lookAt(camEye, camAt, camUp));

		float fov = tan(deg2rad * (scene.config.camera.fov / 2.f));
		...
		for (int y = 0; y < scene.config.height; ++y) {
			for (int x = 0; x < scene.config.width; ++x) {
				...
				if (samplesAccumulated == 0) {
					...
					cvTermData[(x + y * scene.config.width) * 3 + 2] = D.z;
				}
				else {
					float prevSum_x, prevSum_y, prevSum_z;
					prevSum_x = (samplesAccumulated - 1) * cvTermData[(x + y * scene.config.width) * 3];
					prevSum_y = (samplesAccumulated - 1) * cvTermData[(x + y * scene.config.width) * 3 + 1];
					prevSum_z = (samplesAccumulated - 1) * cvTermData[(x + y * scene.config.width) * 3 + 2];

					cvTermData[(x + y * scene.config.width) * 3] = (D.x + prevSum_x) / samplesAccumulated;
					cvTermData[(x + y * scene.config.width) * 3 + 1] = (D.y + prevSum_y) / samplesAccumulated;
					cvTermData[(x + y * scene.config.width) * 3 + 2] = (D.z + prevSum_z) / samplesAccumulated;
				}
			}
		}
		samplesAccumulated++;
		...
		for (auto& object : objects) {
			...
			*/
			...
			if (object.shaderIdx == DIFFUSE_SHADER_IDX) {
				// Diffuse
				// uniforms declared are: albedo, nbTriangles, emitterVertices, windowSize and lightIrradiance
				// albedo
				...
				glUniform1i(nbTrianglesUniform, this->nbTriangles);

				// # of vertices. 
				emitterVerticesUniform = GLuint(glGetUniformLocation(object.shaderID, "emitterVertices"));
				glUniform1fv(emitterVerticesUniform, this->emitterVertexData.size(), this->emitterVertexData.data());

				// window size
				windowSizeUniform = GLuint(glGetUniformLocation(object.shaderID, "windowSize"));
				glUniform2f(windowSizeUniform, this->scene.config.width, this->scene.config.height);
